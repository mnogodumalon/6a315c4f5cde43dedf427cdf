...
export default function VeranstalterPage() {
  ...
  const [records, setRecords] = useState<Veranstalter[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Veranstalter | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Veranstalter | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getVeranstalter());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Veranstalter['fields']) {
  ...
  async function handleUpdate(fields: Veranstalter['fields']) {
    ...
    await LivingAppsService.updateVeranstalterEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteVeranstalterEntry(deleteTarget.record_id);
